...
export const aiApi = {
    // Chat
    sendMessage: async (message: string, conversationId?: string, context?: { caseId?: string; clientId?: string }): Promise<ChatResponse> => {
    ...
    getConversations: async (page = 1, limit = 20): Promise<{ items: Conversation[]; total: number }> => {
    ...
    searchJurisprudence: async (query: string, tribunals?: string[], limit = 10): Promise<SearchResponse> => {
    ...
    indexDocument: async (content: string, documentType: string, metadata?: Record<string, any>): Promise<{ success: boolean; chunksCreated: number }> => {
