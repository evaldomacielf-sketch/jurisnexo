...
export default function ClientsPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => clientsApi.getAll({ search: searchTerm, limit: 100 }),
    });
            ...
            <div className="p-8">
                ...
                {isLoading ? (
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-12 text-center">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Nenhum cliente cadastrado</h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-6">Comece adicionando seu primeiro cliente</p>
                        ...
                        {clients.map((client: ContactDto) => (
                            <div key={client.id} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow">
                                ...
                                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                                    ...
                                    {client.email && <div className="flex items-center gap-2"><Mail className="w-4 h-4" /><span>{client.email}</span></div>}
                                </div>
                            </div>
                        ))}
                    </div>
