...
import type { Client, CreateClientDTO, ClientInteraction, ClientDocument, CreateInteractionDTO } from '../types/client';
...
export const clientsApi = {
    ...
    addInteraction: async (clientId: string, interaction: Omit<CreateInteractionDTO, 'clientId'>) => {
        const { data } = await apiClient.post<ClientInteraction>(`/clients/${clientId}/interactions`, { ...interaction, clientId });
        return data;
    },
    ...
    uploadDocument: async (clientId: string, formData: FormData) => {
        const { data } = await apiClient.post<ClientDocument>(`/clients/${clientId}/documents`, formData, {
            headers: {
