...
export const financeApi = {
  ...
  getCategories: async (type?: string) => {
    const { data } = await apiClient.get<FinanceCategory[]>('/finance/categories', {
      params: { type },
    });
    return data;
  },
  ...
  getCategoryStats: async (startDate?: string, endDate?: string) => {
    const { data } = await apiClient.get<any[]>('/finance/categories/stats', {
  ...
  getAccountBalance: async () => {
    const { data } = await apiClient.get<{ total: number; accounts: any[] }>(
  ...
  getBudgets: async (year: number, month: number) => {
    const { data } = await apiClient.get<BudgetSummary>('/finance/budgets', {
      params: { year, month },
    });
    return data;
  },
  ...
  getTransactions: async (params?: FinanceQuery) => {
    const { data } = await apiClient.get<PaginatedResponse<any>>('/finance/transactions', {
      params,
    });
    return data;
  },
  ...
  getTransactionStats: async (year: number, month: number) => {
    const { data } = await apiClient.get<any>('/finance/transactions/stats/monthly', {
      params: { year, month },
    });
    return data;
  },
  ...
  getReceivables: async (params?: FinanceQuery) => {
    const { data } = await apiClient.get<PaginatedResponse<Receivable>>('/finance/receivables', {
      params,
    });
    return data;
  },
  ...
  recordPayment: async (id: string, payment: RecordPaymentDTO) => {
    const { data } = await apiClient.patch<Receivable>(
      `/finance/receivables/${id}/record-payment`,
      payment
    );
    return data;
  },
  ...
  cancelReceivable: async (id: string, reason?: string) => {
    const { data } = await apiClient.patch<Receivable>(`/finance/receivables/${id}/cancel`, {
      reason,
    });
    return data;
  },
  ...
  getPayables: async (params?: FinanceQuery) => {
    const { data } = await apiClient.get<PaginatedResponse<Payable>>('/finance/payables', {
      params,
    });
    return data;
  },
  ...
  markPayableAsPaid: async (id: string, paymentDate?: string) => {
    const { data } = await apiClient.patch<Payable>(`/finance/payables/${id}/mark-paid`, {
  ...
  getCashFlowProjection: async (days?: number) => {
    const { data } = await apiClient.get<CashFlowProjection>('/finance/reports/cash-flow', {
      params: { days },
    });
    return data;
  },
  ...
  getAgingReport: async (type: 'receivables' | 'payables' = 'receivables') => {
    const { data } = await apiClient.get<AgingReport>('/finance/reports/aging', {
      params: { type },
    });
    return data;
  },
  ...
  getMonthlySummary: async (year?: number, month?: number) => {
    const { data } = await apiClient.get<MonthlySummary>('/finance/reports/monthly', {
      params: { year, month },
    });
    return data;
  },
