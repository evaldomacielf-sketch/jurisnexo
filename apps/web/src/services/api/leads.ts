...
export const leadsApi = {
    ...
    async getLeads(filters?: LeadFilters): Promise<PaginatedResponse<Lead>> {
        try {
            const { data } = await apiClient.get<PaginatedResponse<Lead>>('/crm/leads', { // Adjusted endpoint to match Controller Route
                params: filters,
            });
            return data;
        } catch (error) {
    ...
    async markAsLost(leadId: string, reason: string): Promise<Lead> {
        try {
            const { data } = await apiClient.post<Lead>(`/crm/leads/${leadId}/lost`, { // 'lose' vs 'lost', controller uses 'lost' in summary but route was `lose` in user request 5.1 but my implementation used `lost`? No, wait.
