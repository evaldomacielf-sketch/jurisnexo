...
export default function SLAIndicator({
  ...
  useEffect(() => {
    const calculateRemaining = () => {
      ...
      const lastMessageStr =
        conversation.lastIncomingMessageAt || conversation.lastCustomerMessageAt;
  ...
  const getSizeClasses = () => {
    switch (size) {
      case 'sm':
        return { bar: 'h-1', text: 'text-xs', icon: 'w-3 h-3' };
      case 'lg':
        return { bar: 'h-3', text: 'text-base', icon: 'w-6 h-6' };
      default:
        return { bar: 'h-2', text: 'text-sm', icon: 'w-4 h-4' };
    }
  };
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Progress Bar */}
      <div
        className={`flex-1 ${sizes.bar} min-w-[60px] overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700`}
      >
        <div
      ...
      {/* Status Icon */}
      {isBreached ? (
        <ExclamationTriangleIcon className={`${sizes.icon} animate-pulse text-red-500`} />
      ) : isWarning ? (
...
export function SLABadge({ priority, lastMessageAt }: SLABadgeProps) {
  ...
  if (isBreached) {
    return (
      <span className="inline-flex animate-pulse items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700">
