...
export function InstagramInbox() {
  ...
  const [selectedConversation, setSelectedConversation] = useState<InstagramConversation | null>(
    null
  );
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.map((conv) => (
            <button
              ...
              className={`flex w-full items-start gap-3 border-b border-gray-100 p-4 transition-colors hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800 ${
                ...
                <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                  ...
                  >
                    ...
                    <p className={`mt-1 text-xs ${isOwn ? 'text-blue-100' : 'text-gray-500'}`}>
                      {formatDistanceToNow(new Date(msg.created_time), {
                        locale: ptBR,
                        addSuffix: true,
                      })}
                    </p>
          ...
          <div className="border-t border-gray-200 p-4 dark:border-gray-700">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="flex-1 rounded-full bg-gray-100 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-800"
              />
              <button
                onClick={sendMessage}
                disabled={!messageText.trim() || loading}
                className="rounded-full bg-blue-500 p-2 text-white hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50"
