...
let AuthGuard = class AuthGuard {
    ...
    async canActivate(context) {
        ...
        try {
            // Tenant Check
            const payload = jwt.verify(token, config_1.env.JWT_SECRET);
            ...
            if (tenantId) {
                ...
                if (tenant && tenant.status === 'DISABLED') {
                    throw new common_1.ForbiddenException(`Tenant access disabled: ${tenant.disabled_reason || 'Contact support'}`);
                }
            }
        }
        catch (err) {
