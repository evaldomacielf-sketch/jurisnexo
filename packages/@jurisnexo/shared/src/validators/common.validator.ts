...
export const phoneSchema = z
  .string()
  .regex(/^\+?[1-9]\d{10,14}$/, 'Telefone inválido. Use formato internacional: +5527999999999');

export const cpfSchema = z
  .string()
  .regex(/^\d{3}\.\d{3}\.\d{3}-\d{2}$|^\d{11}$/, 'CPF inválido. Use formato: 000.000.000-00');
...
export const oabSchema = z
  .string()
  .regex(/^[A-Z]{2}\d{4,6}$/, 'OAB inválida. Use formato: UF + número (ex: SP123456)');
