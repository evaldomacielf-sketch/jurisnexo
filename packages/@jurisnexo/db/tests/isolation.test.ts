...
describe('Multi-Tenant Isolation & Security', () => {
  ...
  describe('Audit Events Append-Only', () => {
    ...
    it('Forbids UPDATE on audit events', async () => {
      ...
      await clientA.from('audit_events').update({ event_type: 'MODIFIED' }).eq('id', event.id);
    ...
    it('Forbids DELETE on audit events', async () => {
      ...
      const { data: event } = await clientA.from('audit_events').select().limit(1).single();

      await clientA.from('audit_events').delete().eq('id', event.id);
